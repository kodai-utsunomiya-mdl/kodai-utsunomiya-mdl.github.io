---
title: "CMS (Notes Admin) 構築の記録"
description: "GitHub App + Vercel + Astro APIでCMS相当を自作した手順と設計判断の詳細．"
pubDate: 2026-03-25
draft: false
---
Decap CMSを用いた運用が想定通りに安定せず，最終的にGitHub App + Astro API + Vercelで自作CMS (Notes Admin) を構築した．本稿はその設計判断，構成，実装の詳細を記録したもの．目標は「無料枠で運用可能」「Markdownによる運用」「管理画面から投稿」「数年後でも再構築可能」である．

# 目的と要件
投稿数が増えても破綻しないこと，GitHubが唯一のデータソースになること，複雑な外部依存を避けること，の3点を重視．
...
# 全体構成
構成は以下に落ち着いた．フロントはAstro，データはGitHub，管理画面はAstroのページとして静的に用意し，認証とCRUDだけをAPIで実装する．
...
# 主要なディレクトリ構成
構成の見通しを良くするため，機能単位で分離した．特にAPIとセッション関連は `src/lib` に寄せる．
...
# GitHub Appの作成
GitHub Appはこの仕組みの心臓部．権限の付与は最小限に抑え，Repository Contentsのみ書き込みを許可した．
...
# Installation IDの取得
Installation IDは後で必ず必要になるが，場所が分かりにくい．Appのインストール詳細ページのURLから取得する．
...
# Vercel環境変数
Vercelに以下の環境変数を設定する．ここが一番復旧の鍵になる．
...
# Notes一覧と検索
一覧ページは `/notes/` で，本文も含めた全文検索に対応させた．ヒット件数の表示と巡回は，執筆時に迷わないように調整している．
...
# 結果
Decapを捨てて自作のCMSにしたことで構成が単純化した．GitHubが唯一のデータソースとなり，復旧性が高まった．
